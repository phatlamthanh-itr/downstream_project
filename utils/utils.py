...
def load_data(config, data_type, label_size, downstream = False):
    data_name = config.data_name if downstream == False else config.data_name + "_segmentation"
    ...
    if data_type == "fullts":
        ...
        annotate= "_subseq"
        label_size = f'_{label_size}'
        train_labels = np.load(os.path.join(data_path, f"train_labels{annotate}{label_size}.npy"))
        val_labels = np.load(os.path.join(data_path, f"val_labels{annotate}{label_size}.npy"))
        if downstream:
            test_labels = None
        else:   
